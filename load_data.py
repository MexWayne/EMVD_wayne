...
def load_cvrd_data_org(shift, noisy_level, scene_ind, frame_ind, xx, yy):
...
def load_cvrd_data(shift, noisy_level, scene_ind, frame_ind, xx, yy):
	...
	gt_name = os.path.join(cfg.data_root[1],
						   'indoor_raw_gt/scene{}/ISO{}/frame{}_clean_and_slightly_denoised.tiff'.format(
							   scene_ind, iso_list[noisy_level], frame_list[frame_ind + shift]))
	gt_raw = cv2.imread(gt_name, -1)
	gt_raw_full = gt_raw
	gt_raw_patch = gt_raw_full[yy:yy + cfg.image_height * 2,
				   xx:xx + cfg.image_width * 2]  # 256 * 256
...
def generate_file_list_org(scene_list):
	iso_list = [1600, 3200, 6400, 12800, 25600]
	file_num = 0
	data_name = []
...
def generate_file_list(scene_list):
	...
	for scene_ind in scene_list:
		for iso in iso_list:
			for frame_ind in range(1,8):
				#gt_name = os.path.join('./CRVD_dataset/indoor_raw_gt/scene{}/ISO{}/frame{}_clean_and_slightly_denoised.tiff'.format(scene_ind, iso, frame_ind-1))
				gt_name = os.path.join('./CRVD_dataset/indoor_raw_png/ISO{}/scene{}_frame{}_gt_sRGB.png'.format(iso, scene_ind, frame_ind-1))
				data_name.append(gt_name)
				file_num += 1
...
def decode_data(data_name):
	...
	scene_ind = data_name.split('/')[4].split('_')[0]
	frame_ind = int(data_name.split('/')[4].split('_')[1][5:])
	iso_ind = data_name.split('/')[3]
	...
	yy_list = []
	for shift in range(0, cfg.frame_num):
		gt_name = os.path.join(cfg.data_root[1],'indoor_raw_gt/{}/{}/frame{}_clean_and_slightly_denoised.tiff'.format(scene_ind,iso_ind,frame_list[frame_ind + shift]))
